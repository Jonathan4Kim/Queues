...
let rec assoc (k: 'k) (l: ('k * 'v) list) : 'v option =
  begin match l with
   | [] -> None
   | (x, y)::tl -> if x = k then Some y else assoc k tl
  end
...
let rec cat_option (l: 'a option list) : 'a list =
  begin match l with
    | [] -> []
    | x::tl -> begin match x with 
                | None -> cat_option tl
                | Some(x) -> x::(cat_option tl)
               end
  end
...
let swap (r1: state) (r2: state) : unit =
  let count1 = r1.count in
  let count2 = r2.count in
  r1.count <- count2;
  r2.count <- count1;
  ()
...
let rec contains_alias_option (x: 'a) (l: 'a option list) : bool =
  begin match l with
    | [] -> false
    | y::tl -> begin match y with
                | None -> contains_alias_option x tl
                | Some(z) -> x == z || contains_alias_option x tl
               end
  end
