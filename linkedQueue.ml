...
module LinkedQueue : QueueInterface.QUEUE = struct
  ...
  *)
  ...
  let from_list (l: 'a list) : 'a queue =
    let q = create() in
    let rec loop (l: 'a list) (q: 'a queue) : 'a queue =
      begin match l with
        | [] -> q
        | x::tl -> loop tl (enq x q; q)
      end
    in
    loop l q
  ...
  let delete (elt: 'a) (q: 'a queue) : unit =
    ...
    let rec loop (prev: 'a qnode option) (curr: 'a qnode option) : unit =
      begin match (prev, curr) with
        | (None, None) -> ()
        | (None, Some n) -> if n.v = elt then
                            begin match n.next with
                              | None -> q.head <- None; q.tail <- None
                              | Some y -> q.head <- Some y;
                                          loop None n.next
                            end
                            else loop curr n.next
        | (Some x, None) -> ()
        | (Some x, Some n) -> if n.v = elt then
                              begin match n.next with
                                | None -> q.tail <- Some x; x.next <- None
                                | Some y -> x.next <- n.next; 
                                            loop (Some x) (Some y)
                              end
                              else loop curr n.next
      end
